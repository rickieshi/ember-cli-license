...
var generateHTML = function(npm, bower, other) {
    ...
    _.each(npm, function(stuff) {
        ...
        contentBuffer += `<div id='${stuff.name}${stuff.version}'><h4>${stuff.name} v.${stuff.version}</h4><h5>License</h5><a href=${stuff.licenseLink} target='_blank'>${stuff.license}</a><h5>Repository</h5><a href='${stuff.repository}' target="_blank">${stuff.repository}</a><br/><br/><b>A ${stuff.licenseFile} file was found in component</b><br/><div>${stuff.licenseContent}</div></div><br/><hr>`;
    });
