...
var generateHTML = function (npm, bower, other) {
    ...
    _.each(npm, function (stuff) {
        contentBuffer += `<div id='${stuff.name}${stuff.version}'><h5>${stuff.name}</h5><h5>v.${stuff.version}</h5><h5>Repository</h5><div>${stuff.repository}</div><h5>License File</h5><div>${stuff.licenseFile}</div><h5>License Content</h5><div>${stuff.licenseContent}</div></div><br/><br/>`;
    });
